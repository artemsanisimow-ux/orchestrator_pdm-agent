...
class TestWorkflowDefinitions:
    ...
    def test_research_cycle_agents(self):
        ...
        assert keys == ["cr", "metrics", "interview", "discovery", "ab", "roadmap"]

    def test_retro_cycle_agents(self):
        keys = [k for k, *_ in oc.WORKFLOW_STEPS["retro_cycle"]]
        assert keys == ["retro", "cr", "metrics", "planning", "stakeholder"]

    def test_full_cycle_agents(self):
        keys = [k for k, *_ in oc.WORKFLOW_STEPS["full_cycle"]]
        assert keys == ["cr", "metrics", "interview", "discovery", "prd", "grooming",
                        "planning", "retro", "stakeholder", "roadmap"]
    ...
    def test_full_cycle_is_superset(self):
        ...
        for core in ["cr", "metrics", "interview", "discovery", "planning",
                     "stakeholder", "retro", "roadmap"]:
