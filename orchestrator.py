...
AGENT_DIRS: dict[str, str] = {
    ...
    "interview":   os.getenv("INTERVIEW_DIR",   "../interview-agent"),
    "roadmap":     os.getenv("ROADMAP_DIR",     "../roadmap-agent"),
    "metrics":     os.getenv("METRICS_DIR",     "../metrics-agent"),
}
...
def extract_roadmap(state: OrchState) -> dict:
    """Get roadmap JSON for downstream context."""
    f = find_latest("roadmap_*.json", agent_dirs("roadmap"))
    if not f:
        return {}
    try:
...
def run_metrics(state: OrchState) -> tuple[list[str], dict]:
    mod = load_agent("metrics")
    report = mod.run(source_name="amplitude")
    files = _collect_files("metrics_report_*.md")
    return files, {}


def run_interview(state: OrchState) -> tuple[list[str], dict]:
    cr = extract_cr(state)
    mod = load_agent("interview")
    from interview_agent import TextSource
    # Pass CR opportunities as context for the interview session
    context_text = "\n".join(f"- {o['title']}" for o in cr.get("opportunities", [])[:5])
    _, sid = mod.run(
        source=TextSource(context_text or "General product research session"),
        participant="Research participant",
        product=state.outputs.get("cr", {}).get("data", {}).get("session_id", "Product"),
    )
    return _collect_files("interview_report_*.md"), {"session_id": sid}


def run_roadmap(state: OrchState) -> tuple[list[str], dict]:
    mod = load_agent("roadmap")
    result = mod.run(product="Product")
    files = _collect_files("roadmap_*.md") + _collect_files("roadmap_*.json")
    return files, {}
...
WORKFLOW_STEPS: dict[str, list[Step]] = {
    ...
    "research_cycle": [
        ...
        ("metrics",   "Monitor metrics + detect anomalies",       "Мониторинг метрик + аномалии",      run_metrics),
        ("interview", "Process interview transcripts → OST",      "Обработка интервью → OST",          run_interview),
        ...
        ("roadmap",   "Build roadmap from all signals",           "Роадмап из всех сигналов",          run_roadmap),
    ],
    "retro_cycle": [
...
def print_status() -> None:
    ...
    checks = [
        ...
        ("Interview",   "interview",   "interview_report_*.md"),
        ("Roadmap",     "roadmap",     "roadmap_*.md"),
        ("Metrics",     "metrics",     "metrics_report_*.md"),
    ]
